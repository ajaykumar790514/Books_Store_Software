...
//        
    ...
    public function products($parent_id,$pro_id,$cat_id,$child_cat_id,$search,$classv,$subjectv,$limit=null,$start=null)
    {
        if ($limit!=null) {
            $this->db->limit($limit, $start);
        }
        $this->db
        ->select('t1.*,t2.img,t2.thumbnail,t2.is_cover,t2.id as cover_id, t3.name as publisher_name,t4.qty as qty,t4.mrp as mrp,t4.selling_rate as selling_rate')
        ->from('products_subcategory t1')
        ->join('products_photo t2', 't2.item_id = t1.id AND t2.is_cover = "1"','left')
        ->join('publisher t3', 't3.id = t1.publisher_id AND t3.active = "1" AND t3.is_deleted = "NOT_DELETED"','left')
        ->join('author t5', 't5.id = t1.author_id AND t5.active = "1" AND t5.is_deleted = "NOT_DELETED"','left') 
        ...
        if ($search != 'null'  && $cat_id =='null' || $search != 'null') {
            ...
            $this->db->or_like('t1.isbn_no', $search);
            $this->db->or_like('t1.isbn_no', $search);
            $this->db->or_like('t3.name', $search);
            $this->db->or_like('t5.name', $search);
            $this->db->group_end();
		}
        ...
        if ($classv!= 'null') {
            $this->db->where('t1.class_id', $classv);
        }

        if ($subjectv!= 'null') {
            $this->db->where('t1.subject_id', $subjectv);
        }
    ...
    public function get_property_val_by_id($id)
    {
         ...
         public function fetch_products($id)
        {
            ...
            ->from('cat_pro_maps t3')
            ->join('products_subcategory t1',"t3.pro_id=t1.id")
            ->join('products_photo t2', 't2.item_id = t1.id')           
            // ->join('home_headers_mapping t3', 't3.value = t1.id')           
            ->where(['t2.is_cover' =>'1','t1.is_deleted' =>'NOT_DELETED','t1.active' =>'1','t3.cat_id'=>$id])
            ->get();
            return $query->result();
        }
        ...
        public function get_author($author)
        {
            ...
            ->select('*')
            ->from('author')
            ->where(['active' =>1, 'is_deleted' =>'NOT_DELETED'])
            ->like('name', $author);            
            return $query->get()->result();    
        }
