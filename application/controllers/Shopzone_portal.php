        ...
        switch ($action) {
            case null:
                ...
                case 'tb':
                    ...
                    $data['new_url']        = base_url().'portal-enquiry/create/';
                    $data['tb_url']	  		= base_url().'portal-enquiry/tb';
                    $data['status']         =$this->portal_model->view_status();
                    ...
                    case 'create':
                        if ($p1!=null) {
                            $data['action_url']     = base_url().'portal-enquiry/save/'.$p1;
                            $data['value']          = $this->portal_model->get_row_data1('feedback','id',$p1);
                            $page                   = 'admin/portal/enquiry/status';
                        }
                        $this->load->view($page, $data);
                 break;
                 case 'save':
                    ...
                    if ($this->input->server('REQUEST_METHOD')=='POST') { 
                         ...
                         if ($id!=null) {
                            ...
                            if($this->portal_model->edit_data('feedback',$id,$data)){
                                $return['res'] = 'success';
                                $return['msg'] = 'Updated.';
                            }
                        }
