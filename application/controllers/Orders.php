...
class Orders extends CI_Controller {
    ...
    {
        $data=array();
        $data['courier_company']=$this->input->post('name');
        $data['courier_code']=$this->input->post('code');
        $id=$this->input->post('id');
        $res=$this->orders_model->update_courier($data,$id);
    ...
    public function orderDetails($order_id){
        ...
        if($this->session->has_userdata('logged_in') && $this->session->logged_in === TRUE){
            ...
            $viewData = array(
                                'orderStatus'=>$this->order_status_master_model->getRows(),
                                'orderData'=>$this->orders_model->getRows($order_id),
                                'orderItems'=>$this->order_items_model->getRows($order_id),
                                ...
                                'ordercancelstatus'=>$this->order_status_master_model->getcancelRows($order_id),
                            );
            // echo _prx($viewData);
            // die();  
    ...
    public function updateOrderStatus(){
        ...
        $checkExisting = $this->orders_model->getRows($_POST['item']['id']);
    ...
    public function cancellation_exchange_orders($action=null,$p1=null,$p2=null,$p3=null)
    {
        switch ($action) {
            case null:
                $data['title']          = 'Cancellation Replace Orders';
                $data['tb_url']         = base_url().'cancellation-exchange-orders/tb';
                $data['search']	 		= $this->input->post('search');
                $data['new_url']        = base_url().'cancellation-exchange-orders/create';
                $data['status']  = $this->coupons_model->view_data('cancellations_exchange_status_master');
                $data['payment_status']  = $this->coupons_model->view_data('payment_status_master');
                $page                   = 'shop/cancel_orders/index';
                ...
                case 'tb':
                    ...
                    $config = array();
                    $config["base_url"]         = base_url()."cancellation-exchange-orders/tb/";
                    $config["total_rows"]       = $this->orders_model->cancel_exchange_order();
                    ...
                    $data['rows']           = $this->orders_model->cancel_exchange_order($config["per_page"],$page);
                  //  print_r( $data['rows'] );die();
                    $data['update_url']         = base_url().'cancellation-exchange-orders/create/';
                    $page                       = 'shop/cancel_orders/tb';
                    ...
                    case 'details': 
                      $shop_id     = $_SESSION['user_data']['id'];
		               $shop_details = $this->shops_model->get_shop_data($shop_id);
                        $viewData = array(
                            'orderStatus'=>$this->order_status_master_model->getRowscancel($p1),
                            'orderPaymentStatus'=>$this->order_status_master_model->getRowscancelpayment($p1),
                            'orderData'=>$this->orders_model->getRowscancel($p1),
                            'orderItems'=>$this->order_items_model->getRowscancel($p1),
                            'orderAssign'=>$this->order_assign_deliver_model->getRows(array('conditions'=>array('order_id '=>$p1))),
                            //'delivery_boys'=> $this->shops_delivery_boy_model->delivery_boys(1000,0),
                            'shop_currency'=>$shop_details->currency,
                            'exchangeorder'=>$this->order_status_master_model->exchage_status(),
                            'exchangpaymenteorder'=>$this->order_status_master_model->exchage_payment_status(),
                        );
                        $page                       = 'shop/cancel_orders/order_details';
                        $this->header_and_footer($page, $viewData);
                    break;   
    ...
    public function updatecancelexchangeStatus(){
        
            if($this->orders_model->updatestatusexchange($_POST['item'],array('status'=>$_POST['status'])))
            {
                $data = array(
                    'log_for'     => $_POST['type'],
                    'item_id'     => $_POST['item'],
                ...
                $this->orders_model->Insert('status_logs',$data);
            }
            return TRUE;
    }
    public function updatepaymentstatus(){
        
        if($this->orders_model->updatestatusexchange($_POST['item'],array('payment_status'=>$_POST['status'],'payment_remark'=>$_POST['remark'])))
        {
            $data = array(
                'log_for'     => $_POST['type'],
                'item_id'     => $_POST['item'],
